...
const PartyPage = ({ user }) => {
	...
	// pulls party info, venues, questionnaire, and votes in one go
	// called on mount and after any mutation (vote, midpoint calc, etc)
	async function loadEverything() {
		try {
			...
			try {
				const qData = await questionnaireAPI.getQuestionnaire(partyId);
				if (qData.budget) {
					setQuestionnaire(qData);
					setFormData({
						budget: qData.budget || "medium",
						meeting_type: qData.meeting_type || "casual",
						food_preferences: qData.food_preferences || [],
						dietary_restrictions: qData.dietary_restrictions || [],
						atmosphere: qData.atmosphere || "lively",
						venue_types: qData.venue_types || ["restaurant"],
						travel_weight: qData.travel_weight || 1.0,
					});
				} else {
					// first visit 
			...
			try {
				...
				setVotes(voteLookup);
				setMyVote(voteData.my_vote);
			} catch {}
		} catch (err) {
			setError(err.message || "Couldn't load this party — it may have been deleted");
		} finally {
			setLoading(false);
		}
	}
	...
	const handleCalculateMidpoint = async () => {
		...
		try {
			const result = await partyAPI.getPartyMidpoint(partyId);
