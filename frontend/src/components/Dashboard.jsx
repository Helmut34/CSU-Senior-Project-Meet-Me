...
const Dashboard = ({ user, onLogout, userLocation, onLocationFound }) => {
	...
	async function fetchFriendsList() {
		try {
			...
			setFriendsList(data);
		} catch { // silent fail since we dont know if user has friends yet
		} finally {
	...
	// grab everything we need on first render
	useEffect(() => {
		fetchFriendsList();
		fetchRequests();
		fetchPartyInvites();
	...
	const acceptFriendReq = async (requestId) => {
		try {
			...
			// refresh both lists so the new friend shows up right away
			await fetchRequests();
	...
	const acceptPartyInvite = async (inviteId, partyId) => {
		try {
			...
			toast.success("You're in! Redirecting to the party");
