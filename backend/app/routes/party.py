...
from app.models.models import db, User, Friends, FriendStatus, Party, PartyInvite, PartyQuestionnaire, VenueVote
...
def is_party_member(party, user_id):
	if party.host_user_id == user_id:
		return True
	invite = PartyInvite.query.filter_by(party_id=party.id, invitee_user_id=user_id, status='accepted').first()
...
def create_party():
	...
	# verify all the selected users are actually friends with the host
	for fid in friends_ids:
		friendship = Friends.query.filter(
			((Friends.user_id == current_user.id) & (Friends.friend_id == fid)) |
			((Friends.user_id == fid) & (Friends.friend_id == current_user.id))
		).filter_by(status=FriendStatus.accepted).first()

		if not friendship:
			return jsonify({"error": f"No Longer Friends"}), 400
...
def get_pending_invites():
	invites = PartyInvite.query.filter_by(invitee_user_id=current_user.id, status='pending').all()

	result = []
	for inv in invites:
		p = Party.query.get(inv.party_id)
		host_user = User.query.get(p.host_user_id)

		# grab everyone invited to this party so the user can see who else is going
		all_inv = PartyInvite.query.filter_by(party_id=p.id).all()
		invitee_list = []
		for i in all_inv:
			u = User.query.get(i.invitee_user_id)
			invitee_list.append({
				'id': u.id, 'email': u.email,
				'username': u.username, 'status': i.status.value if hasattr(i.status, 'value') else str(i.status)
			})

		result.append({
		'id': inv.id, 'party_id': p.id,
			'host': {'id': host_user.id, 'email': host_user.email, 'username': host_user.username},
...
def get_party(party_id):
	...
	# make sure the user is actually part of this party before showing details
	if not is_party_member(party, current_user.id):
		return jsonify({'error': 'Not authorized'}), 403

	invites = PartyInvite.query.filter_by(party_id=party_id).all()
	invite_data = []
	for inv in invites:
		user = User.query.get(inv.invitee_user_id)
		invite_data.append({
			'id': inv.id,
			'invitee': {'id': user.id, 'email': user.email, 'username': user.username},
			'status': inv.status.value if hasattr(inv.status, 'value') else str(inv.status)
		})

	host = User.query.get(party.host_user_id)
	...
	return jsonify({
		...
		'meeting_date': party.meeting_date.isoformat() if party.meeting_date else None
	}), 200
...
def get_active_party():
	...
	active_party = None
	if accepted_inv:
		active_party = Party.query.get(accepted_inv.party_id)
	elif hosted:
		active_party = hosted
	...
	host = User.query.get(active_party.host_user_id)

	all_invites = PartyInvite.query.filter_by(party_id=active_party.id).all()
	members = []
	for inv in all_invites:
		u = User.query.get(inv.invitee_user_id)
		members.append({
...
def get_party_midpoint(party_id):
	...
	# collect locations + weights from everyone in the party
	locations = []
	weights = []
	missing_locations = []
	...
	if host.latitude and host.longitude:
		locations.append({'lat': host.latitude, 'lon': host.longitude, 'user': host.username or host.email})

		# check if host filled out the questionnaire for a custom weight
		host_q = PartyQuestionnaire.query.filter_by(party_id=party_id, user_id=host.id).first()
	...
	# now do the same for all accepted invitees
	accepted = PartyInvite.query.filter_by(party_id=party_id, status='accepted').all()

	for inv in accepted:
		invitee = User.query.get(inv.invitee_user_id)

		if invitee.latitude and invitee.longitude:
			locations.append({
				'lat': invitee.latitude, 'lon': invitee.longitude,
				'user': invitee.username or invitee.email
			})
			# grab their questionnaire weight if they filled one out
			inv_q = PartyQuestionnaire.query.filter_by(party_id=party_id, user_id=invitee.id).first()
			weights.append(inv_q.travel_weight if inv_q else inv.midpoint_weight)
		else:
			missing_locations.append(invitee.username or invitee.email)
...
def update_party_weight(party_id):
	...
	if party.host_user_id == current_user.id:
		...
		db.session.commit()
		return jsonify({'message': 'Host weight updated successfully', 'weight': new_weight}), 200
	...
	if invite:
		...
		db.session.commit()
		return jsonify({'message': 'Invitee weight updated successfully', 'weight': new_weight}), 200
...
def get_party_weight(party_id):
	...
	invite = PartyInvite.query.filter_by(party_id=party_id, invitee_user_id=current_user.id).first()
	if invite:
		return jsonify({
			'weight': invite.midpoint_weight, 'role': 'invitee',
...
def vote_for_venue(party_id):
	...
	existing_vote = VenueVote.query.filter_by(party_id=party_id, user_id=current_user.id).first()
	if existing_vote:
	...
	db.session.commit()
	return jsonify({'message': 'Vote recorded', 'venue_place_id': data['venue_place_id']}), 200
...
def get_votes(party_id):
	...
	for v in all_votes:
		...
		if pid not in tally:
			tally[pid] = {
				'venue_place_id': pid, 'venue_name': v.venue_name,
				'venue_address': v.venue_address, 'count': 0
			}
		tally[pid]['count'] += 1

	my_vote = VenueVote.query.filter_by(party_id=party_id, user_id=current_user.id).first()
