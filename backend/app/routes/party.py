...
def get_active_party():
	...
	accepted_inv = PartyInvite.query.filter_by(
		invitee_user_id=current_user.id, status="accepted"
	).first()
...
def get_party_midpoint(party_id):
	...
	if host.latitude and host.longitude:
		...
		host_q = PartyQuestionnaire.query.filter_by(
			party_id=party_id, user_id=host.id
		).first()
	...
	for inv in accepted:
		...
		if invitee.latitude and invitee.longitude:
			locations.append(
				{
					"lat": invitee.latitude,
					"lon": invitee.longitude,
					"user": invitee.username or invitee.email,
				}
			)
			# grab their questionnaire weight if they filled one out
			inv_q = PartyQuestionnaire.query.filter_by(
				party_id=party_id, user_id=invitee.id
			).first()
