...
from app.models.queryHandler import get_accepted_friends, get_pending_friend_requests
...
def add_friend():
    ...
    friend_req = Friends(
        user_id=current_user.id, friend_id=friend_user.id, status=FriendStatus.pending
    )
...
@login_required
def get_friends():
    # now only only query instead of N + 1
    accepted = get_accepted_friends(current_user.id)
    ...
    for fr in accepted:
        ...
        if other.id == current_user.id or other.id in seen_ids:
            continue
...
def get_pending_requests():
    # now only only query instead of N + 1
    pending = get_pending_friend_requests(current_user.id)
