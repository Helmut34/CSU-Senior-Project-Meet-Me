...
def check_party_access(party, user_id, require_accepted=True):
	...
	inv = PartyInvite.query.filter_by(party_id=party.id, invitee_user_id=user_id, **status_filter).first()
...
def submit_questionnaire(party_id):
	...
	if not check_party_access(party, current_user.id):
		return jsonify({'error': 'Unauthorized - you are not a member of this party'}), 403
	...
	existing = PartyQuestionnaire.query.filter_by(party_id=party_id, user_id=current_user.id).first()
	...
	if party.host_user_id == current_user.id:
		...
		inv = PartyInvite.query.filter_by(party_id=party_id, invitee_user_id=current_user.id).first()
		if inv:
...
def get_questionnaire(party_id):
	...
	q = PartyQuestionnaire.query.filter_by(party_id=party_id, user_id=current_user.id).first()
...
def get_matched_venues(party_id):
	...
	return jsonify({
		'midpoint': {'lat': party.midpoint_latitude, 'lon': party.midpoint_longitude},
...
def combine_preferences(questionnaires):
	...
	unanimous_foods = [f for f, cnt in food_counts.items() if num_respondents > 0 and cnt >= num_respondents]
	food_prefs = unanimous_foods if unanimous_foods else sorted(food_counts.keys(), key=lambda f: food_counts[f], reverse=True)
	...
	unanimous_venues = [vt for vt, cnt in venue_counts.items() if venue_respondents > 0 and cnt >= venue_respondents]
	venue_types = unanimous_venues if unanimous_venues else (
		sorted(venue_counts.keys(), key=lambda v: venue_counts[v], reverse=True) if venue_counts else ['restaurant']
	)
